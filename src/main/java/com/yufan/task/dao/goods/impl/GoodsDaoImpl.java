...
public class GoodsDaoImpl implements IGoodsDao {
    ...
    public List<Map<String, Object>> queryTimeGoodsListMap(String goodsId) {
        ...
        sql.append(" g.goods_name,g.sell_count,g.now_money,g.rent_pay_type,p0.param_value as rent_pay_type_name ");
    ...
    private String goodsListSQL(GoodsCondition goodsCondition) {
        ...
        sql.append(" ,g.goods_know,g.rent_pay_type,p0.param_value as rent_pay_type_name,g.goods_type ");
    ...
    public PageInfo loadTimeGoodsList(GoodsCondition goodsCondition) {
        ...
        sql.append(" ,g.goods_know,g.rent_pay_type,p0.param_value as rent_pay_type_name ");
    ...
    public List<Map<String, Object>> mainGoodsListMap(int size, String type) {
        StringBuffer sql = new StringBuffer();
        sql.append(" SELECT g.goods_id,g.title,g.goods_name,g.true_money,g.now_money,CONCAT('" + Constants.IMG_WEB_URL + "',g.goods_img) as goods_img,IFNULL(g.sell_count,0) as sell_count,g.is_single ");
        sql.append(" ,IFNULL(sku.now_money,0) sku_now_money,g.is_zi_yin,IFNULL(tg.id,0) as time_goods_id,IFNULL(tg.time_price,0) as time_price,g.goods_type ");
        sql.append(" ,g.goods_know,g.rent_pay_type,p0.param_value as rent_pay_type_name ");
        sql.append(" from tb_goods g ");
        sql.append(" JOIN tb_shop s on s.shop_id=g.shop_id ");
        sql.append(" LEFT JOIN tb_param p0 on p0.param_code='rent_pay_type' and p0.status=1 and p0.param_key=g.rent_pay_type ");
    ...
    public List<Map<String, Object>> queryTbGoodsListMapByGoodsIds(String goodsIds) {
        ...
        StringBuffer sql = new StringBuffer();
        sql.append(" select goods_id,goods_name,title,goods_img,true_money,now_money,shop_id,is_yuding,get_way,is_invoice,is_putaway,data_index,category_id,property, ");
        sql.append(" is_single,goods_num,is_return,coupon_id,goods_type,is_pay_online,out_code,intro, ");
        sql.append(" deposit_money,peisong_zc_desc,peisong_pei_desc,purchase_price,is_time_goods,limit_num,limit_way, ");
        sql.append(" DATE_FORMAT(limit_begin_time,'%Y-%m-%d %T') as limit_begin_time,level_id,advance_price,bar_code,bar_code_shop,sell_count,is_zi_yin,rent_pay_type ");
    ...
    public List<Map<String, Object>> queryTimeGoodsListMapByTimeGoodsIds(String timeGoodsIds) {
        ...
        StringBuffer sql = new StringBuffer();
        sql.append(" SELECT id as time_goods_id,goods_id,goodssku_id,begin_time,end_time,time_price,goods_store,limit_num,time_way,weight,is_make_sure, ");
        sql.append(" DATE_FORMAT(limit_begin_time,'%Y-%m-%d') as limit_begin_time ");
    ...
    public List<Map<String, Object>> findAllGoodsDetailInfo(String goodsIds) {
        ...
        sql.append(" g.is_invoice,g.is_putaway,g.data_index,g.category_id,g.rent_pay_type, ");
        ...
        sql.append(" ,s.shop_name,p0.param_value as rent_pay_type_name ");
        sql.append(" from tb_goods g ");
        sql.append(" JOIN tb_shop s on s.shop_id = g.shop_id ");
        sql.append(" LEFT JOIN tb_param p0 on p0.param_code='rent_pay_type' and p0.status=1 and p0.param_key=g.rent_pay_type ");
